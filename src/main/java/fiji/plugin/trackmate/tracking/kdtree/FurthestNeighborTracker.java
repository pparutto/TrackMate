...
public class FurthestNeighborTracker extends MultiThreadedBenchmarkAlgorithm implements SpotTracker, Cancelable
{
	...
	public FurthestNeighborTracker( final SpotCollection spots, final Map< String, Object > settings )
	{
		this.spots = spots;
			...
			final Future< Void > future = executors.submit( new Callable< Void >()
			{
				...
				public Void call() throws Exception
				{
					...
					while ( sourceIt.hasNext() )
					{
						...
						{
						    double d = source.squareDistanceTo( targetNodes.get( i ).getValue() );
						    if ( d >= squareDist && d <= maxDistSquare && !targetNodes.get( i ).isVisited() )
						    {
						        squareDist = d;
						        nodeIdx = i;
						    }
						...
						targetNodes.get( nodeIdx ).setVisited( true );
						synchronized ( graph )
						{
							final DefaultWeightedEdge edge = graph.addEdge( source, targetNodes.get( nodeIdx ).getValue() );
